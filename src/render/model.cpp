...
MODEL* ModelLoad( const char *FileName)
{
	...
	for (unsigned int m = 0; m < model->AiScene->mNumMeshes; m++)
	{
		...
		// 頂点バッファ生成
		{
			...
			DX.getDevice()->CreateBuffer(&bd, &sd, &model->VertexBuffer[m]);
		...
		// インデックスバッファ生成
		{
			...
			DX.getDevice()->CreateBuffer(&bd, &sd, &model->IndexBuffer[m]);
	...
	for(int i = 0; i < model->AiScene->mNumTextures; i++)
	{
		...
		CreateShaderResourceView(DX.getDevice(), image.GetImages(), image.GetImageCount(), metadata, &texture);
		assert(texture);
...
void ModelDraw(MODEL* model)
{
	...
	for (unsigned int m = 0; m < model->AiScene->mNumMeshes; m++)
	{
		...
		if (texture != aiString(""))
			DX.getDeviceContext()->PSSetShaderResources(0, 1, &model->Texture[texture.data]);

		// 頂点バッファ設定
		UINT stride = sizeof(Vertex);
		UINT offset = 0;
		DX.getDeviceContext()->IASetVertexBuffers(0, 1, &model->VertexBuffer[m], &stride, &offset);

		// インデックスバッファ設定
		DX.getDeviceContext()->IASetIndexBuffer(model->IndexBuffer[m], DXGI_FORMAT_R32_UINT, 0);

		// ポリゴン描画
		DX.getDeviceContext()->DrawIndexed(mesh->mNumFaces * 3, 0, 0);
	}
}
